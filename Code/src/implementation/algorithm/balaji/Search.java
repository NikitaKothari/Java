...
public class Search {
	...
	/**
	 * Procedure to run binary search on a sorted array of integers Return true
	 * if there is p <= q <= r, with A[q] = x, and false otherwise Runs in time
	 * O(log n), where n = r-p+1
	 *
	 * @param A
	 ...
	 * @return true if there exists q in [p..r] with A[q] = x, and false
	 *         otherwise
	 */
	...
	/**
	 * Implementation of Binary Search algorithm using generics. This can only
	 * be used on a type that implements Comparable itself. Item class can use
	 * it, but ItemExt will not.
	 */
