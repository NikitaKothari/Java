...
public class GradientDescent {
	...
	public static float FindTemp0(float[] h, int[] y, int n, float theta0) {
		float sum = 0;
		for (int i = 0; i < n; i++) {
			sum = sum + (h[i] - y[i]);
		}
	...
	public static float FindTemp1(float[] h, int[] x, int[] y, int n, float theta1) {
		float sum = 0;
		for (int i = 0; i < n; i++) {
			sum = sum + ((h[i] - y[i]) * x[i]);
		}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 5; i++) {
			...
			temp0 = FindTemp0(h, y, n, theta0);
			temp1 = FindTemp1(h, x, y, n, theta1);

			System.out.println("Error is " + error);
			System.out.println("temp0 is " + temp0);
