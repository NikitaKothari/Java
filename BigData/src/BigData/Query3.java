...
public class Query3 {
	 ...
	 */
	public static class BusinessRatingMapper extends Mapper<LongWritable, Text, Text, Text> {
		@Override
	 ...
	 * rating
	 */
	public static class TopTenBusinessReducer extends Reducer<Text, Text, Text, Text> {
		private TreeMap<String, Float> map = new TreeMap<>();
		private RatingComparator ratingComparator = new RatingComparator(map);
		private TreeMap<String, Float> sortedMap = new TreeMap<String, Float>(ratingComparator);
	 ...
	 */
	public static class TopTenBusinessRatingMapper extends Mapper<LongWritable, Text, Text, Text> {
	 ...
	 */
	public static class BusinessDetailMapper extends Mapper<LongWritable, Text, Text, Text> {
		@Override
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			String businessData[] = value.toString().split("::");
	...
	/**
	 * JOB2-Reducer: This reducer class joins the output from the following
	 * mappers 1.TopTenBusinessRatingMapper 2.BusinessDetailMapper and emits the
	 * entire tuple of business.csv file and average rating.
	 */
			...
			for (String topBusiness : topTenBusiness) {
				for (String detail : businessDetails) {
					...
					if (t1BusinessId.equals(t2BusinessId)) {
						context.write(new Text(t1BusinessId),
								new Text(t2Split[1] + "\t" + t2Split[2] + "\t" + t1Split[1]));
						break;
					}
				}
			}
	...
	public static void main(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		...
		if (isJob1Completed) {
			...
			MultipleInputs.addInputPath(job2, new Path(args[2]), TextInputFormat.class,
					TopTenBusinessRatingMapper.class);
			MultipleInputs.addInputPath(job2, new Path(args[1]), TextInputFormat.class, BusinessDetailMapper.class);
